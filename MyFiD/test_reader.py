        ...
        for i, batch in enumerate(dataloader):
            ...
            for k, o in enumerate(outputs):
                ...
                if 'target' in example:
                    score = src.evaluation.ems(ans, example['target'])
                    exactmatch.append(score)

                if opt.write_results:
                    fw.write('\t'.join(['ent1: %s ent2: %s' % (example['target_pair'][0], example['target_pair'][1]), ans, example['target']]) + '\n')
                if opt.write_crossattention_scores:
...
if __name__ == "__main__":
    ...
    if opt.write_results and opt.is_main:
        ...
        write_path = Path(opt.checkpoint_dir) / opt.name / 'final_output.tsv'
